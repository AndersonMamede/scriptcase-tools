(function () {
    ...
    function scriptHint(editor, keywords, getToken, options) {
        ...
        return {
            list: getCompletions(token, context, keywords, editor, options),
            from: {
                line: cur.line, 
                ch: token.start
            },
    ...
    function getCompletions(token, context, keywords, editor, options) {
        var found = [], start = token.string;
        function maybeAdd(data) {
            var str = data.text || data;
            if (str.indexOf(start) == 0) found.push(data);
        }
        ...
        if (context) {
            ...
            forEach(anywordHint(editor, options), maybeAdd);
            forEach(ScriptCaseMacros(), maybeAdd);
        }
        return found;
    }
    
    // For anywordHint to work, anyword-hint.js must be included in the page
    function anywordHint(editor, options){
        if(CodeMirror.hint.anyword){
            return CodeMirror.hint.anyword(editor, options).list;
        }else{
            return [];
        }
    }
